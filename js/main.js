document.addEventListener('DOMContentLoaded', function () {
    ...
    if (reservationCanvas) {
        ...
        var reservationSequenceUrls = new Array(reservationSequenceFrameCount).fill().map(function (unused, index) {
            ...
            var fileName = 'dg (' + frameIndex + ').JPG';
    ...
    var productSection = document.querySelector('#section6');
    var productModal = productSection ? productSection.querySelector('.product-modal') : null;
    var productContent = productModal ? productModal.querySelector('.product-content') : null;
    var productOverlay = productModal ? productModal.querySelector('.product-overlay') : null;
    var productCards = productSection ? gsap.utils.toArray(productSection.querySelectorAll('.product-card')) : [];
    var productImages = productSection ? gsap.utils.toArray(productSection.querySelectorAll('.product-image')) : [];
    var activeProductIndex = null;
    ...
    function isSectionPinDisabled(section) {
        if (!section) {
            return false;
        }
        return section.getAttribute('data-pin') === 'false';
    }

    function createFreeSectionTrigger(section) {
    ...
    sections.forEach(function (section) {
        ...
        if (isSectionPinDisabled(section)) {
            createFreeSectionTrigger(section);
        } else {
    ...
    function closeProductModal() {
        if (activeProductIndex === null || !productModal) {
            return;
        }
        var image = productImages[activeProductIndex];
        var card = productCards[activeProductIndex];
        if (!image || !card) {
            activeProductIndex = null;
            return;
        }
        var state = window.Flip ? window.Flip.getState(image) : null;
        card.appendChild(image);
        activeProductIndex = null;
        gsap.set(image, { zIndex: 2001 });
        var fadeTargets = productOverlay ? [productModal, productOverlay] : [productModal];
        gsap.to(fadeTargets, {
            autoAlpha: 0,
            ease: 'power1.inOut',
            duration: 0.35
    ...
    function openProductModal(index) {
        ...
        gsap.set(productModal, { autoAlpha: 1 });
        if (state && window.Flip) {
            window.Flip.from(state, {
                duration: 0.7,
                ease: 'power1.inOut'
        ...
        if (productOverlay) {
            gsap.to(productOverlay, { autoAlpha: 0.65, duration: 0.35 });
        }
    }

    function setupProductGallery() {
        if (!productSection || !productImages.length) {
            return;
        }
